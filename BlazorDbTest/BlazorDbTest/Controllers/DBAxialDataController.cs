...
namespace BlazorDbTest.Controllers {
  ...
  public class DBAxialDataController : ControllerBase {
    ...
    public List<AxialList> GetOptAxialList(string patientId) {
      ...
      try {
        ...
        if (uuid == string.Empty) {
          ...
          Query += DBCommonController._table(DBCommonController.DB_TableNames[(int)DBCommonController.eDbTable.EXAM_OPTAXIAL]);
          Query += ".";
          Query += DBCommonController._col(COLNAME_ExamOptaxialList[(int)eExamOptAxial.fitting_id]);
          Query += " = ";
          Query += DBCommonController._val(fittingId.ToString());
          Query += " )";
          Query += " ORDER BY ";
          ...
          DataSource = SetAxialList(patientId, AxialDataSource.ToArray(), sqlConnection);
        }
      } catch {
      } finally {
        // PostgreSQL Server 通信切断
          ...
          for (int i = 0; i < axialDataList.Length; i++) {
                ...
                if (axialDataList[i].EyeId == EyeType.right) {
                  ...
                  if (!list[j].IsRManualInput) {
                    if (list[j].RAxial == null) {
                      ...
                      list[j].RAxial = axialDataList[i].Axial[selectId];
                      list[j].IsRManualInput = (axialDataList[i].DeviceId == axmId);
                      isExist = true;
                      break;
                  ...
                  if (!list[j].IsLManualInput) {
                    if (list[j].LAxial == null) {
                      ...
                      list[j].LAxial = axialDataList[i].Axial[selectId];
                      list[j].IsLManualInput = (axialDataList[i].DeviceId == axmId);
                      isExist = true;
                      break;
                    } else if (list[j].ExamDateTime < axialDataList[i].ExamDateTime) {
                      // 左眼かつ同じ測定時間が新しい
            ...
            if (!isExist) {
              ...
              if (axialDataList[i].EyeId == EyeType.right) {
                ...
                var.RAxial = axialDataList[i].Axial[0];
                var.IsRManualInput = (axialDataList[i].DeviceId == axmId);
              } else if (axialDataList[i].EyeId == EyeType.left) {
                var.LExamID = axialDataList[i].ID;
                var.LAxial = axialDataList[i].Axial[0];
                var.IsLManualInput = (axialDataList[i].DeviceId == axmId);
              }
              list.Add(var);
            }
          }
    ...
    public static ExamOptaxialRec MakeOptaxialRec(int examId, string posEye, NpgsqlConnection sqlConnection) {
      ...
      int selectId = DBCommonController.Select_SelectTypeID(sqlConnection, DBConst.SELECT_TYPE[(int)DBConst.SelectType.none]);
      ...
      try {
        ...
        recOpax.device_id = DBCommonController.Select_Device_ID(sqlConnection, DBConst.AxmDeviceType);
    ...
    /// <returns></returns>
    public static double GetLatestAxialData(string pt_uuid, int eye_id, DateOnly examdate, double min, double max, int selectId, int fittingId, NpgsqlConnection sqlConnection) {
      double axial_mm = -1;
      ...
      try {
        ...
        int pos = (selectId >= 1) ? selectId - 1: 0;
        if (DataTable.Rows.Count > 0) {
          ...
          if (DBCommonController._objectToDateOnly(DataTable.Rows[0][COLNAME_ExamOptaxialList[(int)eExamOptAxial.measured_at]]) == examdate) {
            ...
            axial_mm = Convert.ToDouble(axialList[pos]);
          }
        }
      } catch {
      } finally {
