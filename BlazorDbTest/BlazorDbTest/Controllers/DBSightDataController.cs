...
namespace BlazorDbTest.Controllers {
  ...
  public class DBSightDataController : ControllerBase {
    ...
    // 視力測定値削除
    [HttpPost("DeleteSightData")]
    public void DeleteSightData([FromBody] int examId) {
      try {
        DBAccess dbAccess = DBAccess.GetInstance();
    ...
    // 視力測定値移動
    [HttpPost("MoveSightData")]
    public void MoveSightData([FromBody] MoveExamData conditions) {
      try {
        if (conditions == null) return;
